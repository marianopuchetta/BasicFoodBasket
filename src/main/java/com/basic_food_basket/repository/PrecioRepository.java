...
public interface PrecioRepository extends JpaRepository<Precio, Long> {
    ...
    @Query("SELECT p FROM Precio p WHERE p.id IN (" +
           "SELECT MAX(p2.id) FROM Precio p2 WHERE p2.producto.id IN :productoIds AND p2.scrapeado = true GROUP BY p2.producto.id)")
    List<Precio> findLastScrapeadoByProductoIds(@Param("productoIds") List<Long> productoIds);

    @Query("SELECT p FROM Precio p WHERE p.fecha BETWEEN :startDate AND :endDate AND p.producto.tipoCanasta = :tipoCanasta")
    List<Precio> findByFechaBetweenAndTipoCanasta(
            @Param("startDate") LocalDate startDate,
            @Param("endDate") LocalDate endDate,
            @Param("tipoCanasta") TipoCanasta tipoCanasta);
    @Query("SELECT p FROM Precio p JOIN FETCH p.producto pr JOIN FETCH pr.supermercado WHERE p.fecha BETWEEN :start AND :end")
    List<Precio> findPreciosConRelacionesCompletas(
        @Param("start") LocalDate start,
        @Param("end") LocalDate end);
    
    @Query("SELECT p FROM Precio p JOIN FETCH p.producto WHERE p.fecha BETWEEN :start AND :end")
    List<Precio> findPreciosConProducto(@Param("start") LocalDate start, 
                                       @Param("end") LocalDate end);
    
    @Query("SELECT p FROM Precio p JOIN FETCH p.producto WHERE p.fecha IS NOT NULL")
    List<Precio> findAllPreciosConFecha();
}
